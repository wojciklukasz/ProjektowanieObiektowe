...
describe('Form test', () => {
    ...
    it('Adds Sprzątanie garażu to basket and checks if amount to pay is correct', () => {
        cy.get('[href="/services/"]').click()
        cy.get(':nth-child(4) > pre > button').click()
        cy.get('[href="/order/"]').click()
        cy.get('[href="/payment/"]').click()
        cy.contains('Do zapłaty: 25')
    })

    it('Removes Sprzątanie garażu from basket and checks if amount to pay ib back to 0', () => {
        cy.get('[href="/services/"]').click()
        cy.get(':nth-child(4) > pre > button').click()
        cy.get('[href="/order/"]').click()
